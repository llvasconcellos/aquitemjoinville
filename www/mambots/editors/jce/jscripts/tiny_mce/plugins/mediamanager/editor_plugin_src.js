...
tinyMCE.importPluginLanguagePack('mediamanager');

var TinyMCE_MediaPlugin = {
	getInfo : function() {
		return {
			longname : 'Media Manager',
			author : 'Moxiecode Systems / Ryan Demmer',
			authorurl : 'http://www.cellardoor.za.net',
			infourl : 'http://www.cellardoor.za.net/index2.php?option=com_content&amp;task=findkey&amp;pop=1&amp;lang=en&amp;keyref=mediamanager.about',
			version : '1.1.5'
		};
	},
	...
	initInstance : function(inst) {
		if (!tinyMCE.settings['media_skip_plugin_css'])
			tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/mediamanager/css/content.css");
	},

	getControlHTML : function(cn) {
		switch (cn) {
			case "mediamanager":
				return tinyMCE.getButtonHTML(cn, 'lang_mediamanager_desc', '{$pluginurl}/images/mediamanager.gif', 'mceMedia');
		}
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		...
		switch (command) {
			case "mceMedia":
				tinyMCE.openWindow({
						file : tinyMCE.getParam('site') + '/index2.php?option=com_jce&no_html=1&task=plugin&plugin=mediamanager&file=manager.php',
						width : 750 + tinyMCE.getLang('lang_mediamanager_delta_width', 0),
						height : 620 + tinyMCE.getLang('lang_mediamanager_delta_height', 0)
					}, {
						editor_id : editor_id,
						inline : "yes"
				});
	...
	_createImg : function(cl, d, n) {
		...
		ne.width = tinyMCE.getAttrib(n, 'width');
		ne.height = tinyMCE.getAttrib(n, 'height');
		ne.align = tinyMCE.getAttrib(n, 'align');
		ne.className = cl;
	...
	_getEmbed : function(cls, cb, mt, p, at) {
		...
		// insert baseURL on WMP if it's an absolute, local URL. Else delete baseURL
		if (mt == 'application/x-mplayer2' && p.src.indexOf(tinyMCE.settings['base_href']) != -1) {
			p.baseurl = tinyMCE.settings['base_href'].substr(0, tinyMCE.settings['base_href'].length - 1);
		}
		else {
			delete p.baseurl;
		}	
		// generate object
		for (n in p) {
			if (p[n] != "undefined" && typeof(p[n]) != "function") {
				// Add url parameter on WMP (src is still needed for JCE)
				if (n == 'src' && mt == 'application/x-mplayer2') {
					h += '<param name="url" value="' + p[n] + '" />';
				}
				// don't want url twice
				if (n != 'url') {
...
tinyMCE.addPlugin("mediamanager", TinyMCE_MediaPlugin);