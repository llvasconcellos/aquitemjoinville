...
class content_archive_section_menu_html {
	...
	function editSection( &$menu, &$lists, &$params, $option ) {
			...
			if ( !$menu->id ) {
				...
				if ( form.name.value == '' ) {
					if ( form.componentid.value == 0 ) {
						form.name.value = "Todas as Seções";
					} else {
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Editar' : 'Adicionar';?> Blog - Arquivo de Conteúdo da Seção
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					<td valign="top" align="right">Seção:</td>
					<td colspan="2">
