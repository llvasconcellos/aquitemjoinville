...
class mosInstaller {
	...
	*/
	function extractArchive() {
		...
		if (eregi( '.zip$', $archivename )) {
			...
			if($ret == 0) {
				$this->setError( 1, 'Erro irrecuperável "'.$zipfile->errorName(true).'"' );
				return false;
			}
		} else {
			...
			if (!$archive->extractModify( $extractdir, '' )) {
				$this->setError( 1, 'Erro de extração' );
				return false;
			}
		}
	...
	*/
	function findInstallFile() {
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, 'ERRO: Não foi possível encontrar um arquivo XML de instalação Joomla! no pacote.' );
			return false;
		} else {
			$this->setError( 1, 'ERRO: Não foi possível encontrar um arquivo XML de instalação no pacote.' );
			return false;
		}
	}
	/**
	...
	*/
	function readInstallFile() {
		...
		if ($this->installFilename() == "") {
			$this->setError( 1, 'Não foi especificado um nome de arquivo' );
			return false;
		}
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, 'O arquivo:"' . $this->installFilename() . '" não é um arquivo de instalação Joomla! válido' );
			return false;
		}
	...
	*/
	function install() {
		die( 'O Método "instalar" não pode ser ativado pela classe ' . strtolower(get_class( $this )) );
	}
	/**
	* Abstract uninstall method
	*/
	function uninstall() {
		die( 'O Método "desinstalar" não pode ser ativado pela classe ' . strtolower(get_class( $this )) );
	}
	/**
	* return to method
	*/
	...
	*/
	function preInstallCheck( $p_fromdir, $type ) {
		...
		if (!$this->readInstallFile()) {
			$this->setError( 1, 'Arquivo de instalação não encontrado:<br />' . $this->installDir() );
			return false;
		}

		if ($this->installType() != $type) {
			$this->setError( 1, 'O arquivo de setup XML não é para um "'.$type.'".' );
			return false;
		}
			...
			if (basename( $file->getText() ) != $file->getText()) {
				...
				if ($adminFiles){
					if (!mosMakePath( $this->componentAdminDir(), $newdir )) {
						$this->setError( 1, 'Erro ao criar diretório "' . ($this->componentAdminDir()) . $newdir . '"' );
						return false;
					}
				} else {
					if (!mosMakePath( $this->elementDir(), $newdir )) {
						$this->setError( 1, 'Erro ao criar diretório "' . ($this->elementDir()) . $newdir . '"' );
						return false;
					}
				}
			}
