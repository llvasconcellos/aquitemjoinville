...
function uploadPackage( $installerClass, $option, $element, $client ) {
	...
	if (!(bool)ini_get('file_uploads')) {
		HTML_installer::showInstallMessage( "O instalador não pode continuar antes que o envio dos arquivos esteja concluido. Por favor, use a instalação a partir do diretório.",
	...
	if(!extension_loaded('zlib')) {
		HTML_installer::showInstallMessage( "O instalador não pode continuar antes que o zlib esteja habilitado",
			'Installer - Error', $installer->returnTo( $option, $element, $client ) );
		exit();
	}
	...
	if (!$userfile) {
		HTML_installer::showInstallMessage( 'Nenhum arquivo selecionado', 'envio novo módulo - erro',
