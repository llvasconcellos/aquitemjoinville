...
function writableCell( $folder ) {
	...
	echo '<td align="left">';
	echo is_writable( $GLOBALS['mosConfig_absolute_path'] . '/' . $folder ) ? '<b><font color="green">Permissão para escrita</font></b>' : '<b><font color="red">Sem permissão para escrita</font></b>' . '</td>';
	echo '</tr>';
}
	...
	function showInstallForm( $title, $option, $element, $client = "", $p_startdir = "", $backLink="" ) {
		...
		function submitbutton3(pressbutton) {
			...
			if (form.userfile.value == ""){
				alert( "Por favor, selecione um diretório" );
			} else {
				form.submit();
			}
		}
		...
		<tr>
			...
			<input class="button" type="submit" value="Enviar o Arquivo e Instala" />
			</td>
		</tr>
		</table>
		...
		<tr>
			...
			<input type="button" class="button" value="Instalar" onclick="submitbutton3()" />
			</td>
		</tr>
		</table>
	...
	*/
	function showInstallMessage( $message, $title, $url ) {
		...
		<tr>
			...
			[&nbsp;<a href="<?php echo $url;?>" style="font-size: 16px; font-weight: bold">Continuar...</a>&nbsp;]
			</td>
		</tr>
		</table>
		<?php
