...
class mosInstallerMambot extends mosInstaller {
	...
	*/
	function uninstall( $id, $option, $client=0 ) {
		...
		if (trim( $row->folder ) == '') {
			HTML_installer::showInstallMessage( 'Campo de Diretório vazio, Não foi possível remover arquivos', 'Desinstalar -  erro',
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage( $row->name .' é um elemento do sistema e não pode ser desinstalado.<br />Será necessário despublicá-lo caso não pretenda continuar a utilizá-lo',
