...
class HTML_Media {
					...
					<tr>
						<td>
							...
							<tr>
								...
								<td align="right">
									Enviar Arquivo <small>[ Max = <?php echo ini_get( 'post_max_size' );?> ]</small>
									&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<tr>
			<td>
				<div align="center" style="font-size:small;font-weight:bold;color:#CC0000;font-family: Helvetica, sans-serif;">
					Configuração com problema: &quot;<?php echo $BASE_DIR.$BASE_ROOT; ?>&quot; não insista.
				</div>
			</td>
		</tr>
		</table>
	...
	function show_image($img, $file, $info, $size, $listdir) {
		...
		$overlib .= '</tr>';
		$overlib .= '</table>';
		$overlib .= '<br/> *Clique na imagem para ampliar*';
		$overlib .= '<br/> *Clique no lápis para ver o código de imagem*';
		?>
	...
	function imageStyle($listdir) {
		...
		function deleteFolder(folder, numFiles) {
			if(numFiles > 0) {
				alert("Há "+numFiles+" arquivo(s) no diretório \""+folder+"\".\n\nPor favor, apague primeiro todos os arquivos do diretório \""+folder+"\".");
				return false;
			}

			if(confirm("Deletar diretório \""+folder+"\"?"))
			return true;
