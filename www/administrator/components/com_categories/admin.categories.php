...
function showCategories( $section, $option ) {
	...
	if (intval( $section ) > 0) {
		...
		$section_name = 'Conteúdo: '. $section_name;
...
function editCategory( $uid=0, $section='' ) {
	...
	if (!$sections && $type != 'other') {
		echo "<script> alert('Você precisa ter uma seção pelo menos antes de criar uma categoria'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	if ($row->checked_out && $row->checked_out != $my->id) {
		mosRedirect( 'index2.php?option=categories&section='. $row->section, 'A categoria'. $row->title .' está sendo editada atualmente por outro administrador' );
	}
	...
	$types[] = mosHTML::makeOption( '', 'Escolher Tipo' );
	if ($row->section == 'com_contact_details') {
		$types[] = mosHTML::makeOption( 'contact_category_table', 'Contato por categoria em tabela' );
	} else
	if ($row->section == 'com_newsfeeds') {
		$types[] = mosHTML::makeOption( 'newsfeed_category_table', 'Newsfeed por categoria em tabela' );
	} else
	if ($row->section == 'com_weblinks') {
		$types[] = mosHTML::makeOption( 'weblink_category_table', 'Weblink por categoria em tabela' );
	} else {
		$types[] = mosHTML::makeOption( 'content_category', 'Conteúdo por categoria em tabela' );
		$types[] = mosHTML::makeOption( 'content_blog_category', 'Conteúdo por categoria em blog' );
		$types[] = mosHTML::makeOption( 'content_archive_category', 'Arquivo por categoria em blog' );
	} // if
	...
	if ( $row->section > 0 || $row->section == 'content' ) {
		...
		$folders[] 	= mosHTML::makeOption( '*2*', 'Usar configurações da Seção'  );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '*1*', 'Todos'  );
		$folders[] 	= mosHTML::makeOption( '*0*', 'Nenhum' );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '/' );
...
function saveCategory( $task ) {
	...
	switch ( $task ) {
		...
		case 'apply':
			$msg = 'Modificações em categoria realizadas com sucesso!';
...
function removeCategories( $section, $cid ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Selecione uma categoria para apagar'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( "\', \'", $err );
		$msg = 'Categoria(s): '. $cids .' não pode(m) ser removida(s), pois possui(em) registro(s)';
...
function publishCategories( $section, $categoryid=null, $cid=null, $publish=1 ) {
	...
	if (count( $cid ) < 1) {
		...
		echo "<script> alert('Selecione uma categoria para $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function moveCategorySave( $cid, $sectionOld ) {
	...
	$msg = ( (count($cid) - 1) ? 'Categories' : 'Category' ) .' movida para '. $sectionNew->name;
...
function copyCategorySave( $cid, $sectionOld ) {
	...
	foreach( $cid as $id ) {
		...
		$category->id 		= NULL;
		$category->title 	= 'Cópia de '. $category->title;
		$category->name 	= 'Cópia de '. $category->name;
	...
	$msg = ( (count($cid) - 1) ? 'Categories' : 'Category' ) .' copiada para '. $sectionNew->name;
	mosRedirect( 'index2.php?option=com_categories&section='. $sectionOld .'&mosmsg='. $msg );
}
...
function menuLink( $id ) {
	...
	$msg = $name .' ( '. $menutype .' ) no menu: '. $menu .' criado com sucesso';
