...
function showUsers( $option ) {
	...
	. "\n AND name != 'USERS'"
	;
	$types[] = mosHTML::makeOption( '0', '- Escolher Grupo -' );
	$database->setQuery( $query );
	$types = array_merge( $types, $database->loadObjectList() );
	$lists['type'] = mosHTML::selectList( $types, 'filter_type', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', "$filter_type" );

	// get list of Log Status for dropdown filter
	$logged[] = mosHTML::makeOption( 0, '- Escolher Estado de Ligação - ');
	$logged[] = mosHTML::makeOption( 1, 'Autenticado');
...
function saveUser( $task ) {
		...
		if ( $row->gid != $original->gid ) {
			if ( $original->gid == 25 ) {
				...
				if ( $count <= 1 ) {
					...
					echo "<script> alert('Não pode alterar o Grupo deste usuário já que é o único Super Administrador ativo no seu site'); window.history.go(-1); </script>\n";
					exit();
				}
			}
			...
			if (( $user_group == 'super administrator' && $my->gid != 25) ) {
				...
				echo "<script> alert('Não pode alterar o Grupo destes Usuárioss já que você não é Super Administrator do site'); window.history.go(-1); </script>\n";
				exit();
			} else if ( $my->gid == 24 && $original->gid == 24 ) {
				// disallow change of super-Admin by non-super admin
				echo "<script> alert('Não pode alterar o Grupo de outro Administrator já que você não é Super Administrator do site'); window.history.go(-1); </script>\n";
				exit();
	...
	if ( $row->gid == 25 && $my->gid != 25 ) {
		...
		echo "<script> alert('Não pode criar um usuário com este nível de Grupo, só os Super Administradores têm esta capacidade'); window.history.go(-1); </script>\n";
		exit();
	}
	*/
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Alteração realizada com sucesso!: '. $row->name;
			mosRedirect( 'index2.php?option=com_users&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Usuário salvo com sucesso!: '. $row->name;
...
function removeUsers( $cid, $option ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Selecione um item para apagar'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( !$msg && count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			// cannot delete Super Admin where it is the only one that exists
				$msg = "Não pode eliminar este Super Administrador já que é o único Super Administrador do seu site";
			} else {
				// delete user
				$obj->delete( $id );
				$msg = $obj->getError();
	...
	if ( count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			if ( $this_group == 'super administrator' && $my->gid != 25 ) {
				$msg = "Você não pode apagar um Super Administrador";
 			} else if ( $id == $my->id ){
 				$msg = "Você não pode se apagar!";
 			} else if ( ( $this_group == 'administrator' ) && ( $my->gid == 24 ) ){
 				$msg = "Você não pode apagar outro `Administrator ` só Super Administradores têm essa capacidade";
			} else {
				$obj->load( $id );
				$count = 2;
				if ( $obj->gid == 25 ) {
				...
				if ( $count <= 1 && $obj->gid == 25 ) {
				// cannot delete Super Admin where it is the only one that exists
					$msg = "Não pode eliminar este Super Administrador pois é o único Super Administrador ativo no seu site";
				} else {
					// delete user
					$obj->delete( $id );
					$msg = $obj->getError();
...
function changeUserBlock( $cid=null, $block=1, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script type=\"text/javascript\"> alert('Selecione um item para $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function logoutUser( $cid=null, $option, $task ) {
	...
	if ( is_array( $cid ) ) {
		...
		// check to see whether a Administrator is attempting to log out a Super Admin
		if ( $my->gid == 24 && $temp->gid == 25 ) {
			echo "<script> alert('Você não pode terminar a sessão de um Super Administrador'); window.history.go(-1); </script>\n";
			exit();
		}
		$ids = 'userid=' . (int) $cid;
	}
