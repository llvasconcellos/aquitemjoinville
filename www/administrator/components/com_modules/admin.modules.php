...
function saveModule( $option, $client, $task ) {
	...
	switch ( $task ) {
		...
		case 'save':
		default:
			$msg = 'Modulo gravado com sucesso!: '. $row->title;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client, $msg );
			break;
	}
}
...
function removeModule( &$cid, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Selecione um module para apagar'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	foreach ($rows as $row) {
		...
		if ( $row->module == 'mod_mainmenu' ) {
			if ( strstr( $row->params, 'mainmenu' ) ) {
				echo "<script> alert('Você não pode apagar o módulo mod_mainmenu que exibe o \'mainmenu\' pois pertence ao core do Joomla!'); window.history.go(-1); </script>\n";
				exit;
			}
		}
	}
	...
	if (count( $err )) {
		...
		echo "<script>alert('Módulo(s): \'$cids\' não podem ser apagados pois eles pertence ao core do Joomla!. Eles só poderam ser removidos quando listados em desistalar módulos.');</script>\n";
	}
...
function publishModule( $cid=null, $publish=1, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		...
		echo "<script> alert('Selecione um módulo para $action'); window.history.go(-1);</script>\n";
		exit;
	}
