...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen('../configuration.php','w')){
		...
		fclose($fp);
		mosRedirect('index2.php?option=com_languages',"Configuração realizada com sucesso! $p_lname");
	} else {
		mosRedirect('index2.php?option=com_languages','Erro!');
	}
...
function removeLanguage( $cid, $option, $client = 'admin' ) {
	...
	if ($cur_language == $cid) {
		mosErrorAlert( "Você não pode apagar idioma em uso." );
	}
...
function saveLanguageSource( $option ) {
	...
	if (!$language) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operação Falhou: Nenhum idioma especificado." );
	}
	if (!$filecontent) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operação Falhou: Conteúdo vazio." );
	}
	...
	if (is_writable( $file ) == false) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operação Falhou: O arquivo não possui permissão para escrita." );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Operação Falhou: Não foi possível abrir o arquivo para escrita." );
	}
}
?>
