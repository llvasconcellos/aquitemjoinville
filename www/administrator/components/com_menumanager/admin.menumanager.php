...
function saveMenu() {
	...
	if ( $old_menutype == 'mainmenu' ) {
		if ( $menutype != 'mainmenu' ) {
			echo "<script> alert('Você não pode renomear o \'mainmenu\' como isto romperá a operação formal de Joomla!'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	if (strstr($menutype, '\'')) {
		echo "<script> alert('Este nome não pode conter uma \''); window.history.go(-1); </script>\n";
		exit;
	}
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menutype ) {
			echo "<script> alert('Já existe um menu com esse nome - você tem que entrar com um nome para o menu diferente'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	switch ( $new ) {
		...
		// change menutype of all menuitems using old menutype
			...
			$msg = 'Menu Itens & Modulos atualizados';
			break;
	}
...
function deleteConfirm( $option, $type ) {
	...
	if ( $type == 'mainmenu' ) {
		echo "<script> alert('Você não pode apagar o \'mainmenu\' o menu pertende ao core do joomla!'); window.history.go(-1); </script>\n";
		exit();
	}
...
function copyMenu( $option, $cid, $type ) {
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menu_name ) {
			echo "<script> alert('Já existe um menu com esse nome - você tem que entrar com um nome diferente para o menu'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	$msg = 'Cópia do menu `'. $type .'` foi criada, consistindo em '. $total .' itens';
	mosRedirect( 'index2.php?option=' . $option, $msg );
}
