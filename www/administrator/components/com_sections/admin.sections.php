...
function editSection( $uid=0, $scope='', $option ) {
	...
	if ($row->isCheckedOut( $my->id )) {
		$msg = 'A seção '. $row->title .' está sendo editado atualmente por outro administrador';
	...
	$types[] = mosHTML::makeOption( 'content_section', 'Seção em Lista' );
	$types[] = mosHTML::makeOption( 'content_blog_section', 'Seção em Blog' );
	$types[] = mosHTML::makeOption( 'content_archive_section', 'Seção de arquivo em Blog' );
...
function removeSections( $cid, $scope, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Selecione a seção para deletar'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( ', ', $err );
		$msg = 'Seção(s): '. $cids .' não pode ser removido, pois contêm categorias';
		mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
	}
	...
	$names = implode( ', ', $name );
	$msg = 'Seção(s): '. $names .' apagada com sucesso!';
	mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
}
...
function publishSections( $scope, $cid=null, $publish=1, $option ) {
	...
	if ( !is_array( $cid ) || count( $cid ) < 1 ) {
		...
		echo "<script> alert('Selecione a seção para $action'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( $publish ) {
		if ( !$count ){
			echo "<script> alert('Não é possivel publicar uma seção vazia $count'); window.history.go(-1);</script>\n";
			return;
		}
	}
