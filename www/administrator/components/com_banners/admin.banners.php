...
function editBanner( $bannerid, $option ) {
  ...
  if ( $bannerid ){
	...
	$clientlist[] 	= mosHTML::makeOption( '0', 'Escolher Cliente', 'cid', 'name' );
...
function editBannerClient( $clientid, $option ) {
	...
	if ($row->checked_out && $row->checked_out != $my->id) {
		$msg = 'O Cliente [ '. $row->name. ' ] está, sendo editado atualmente por outra pessoa.';
...
function removeBannerClients( $cid, $option ) {
	...
	for ($i = 0; $i < count($cid); $i++) {
		...
		if ($count != 0) {
			mosRedirect( "index2.php?option=$option&task=listclients",
			"Não foi possível deletar o cliente neste momento porque ele ainda tem banner sendo exibido" );
		} else {
