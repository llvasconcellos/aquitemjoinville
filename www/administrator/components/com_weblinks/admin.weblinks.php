...
function editWeblink( $option, $id ) {
	...
	if ($row->isCheckedOut( $my->id )) {
		mosRedirect( 'index2.php?option='. $option, 'O módulo $row->title está sendo atualmente editado por outro administrador.' );
	}
...
function removeWeblinks( $cid, $option ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Selecione um artigo para apagar'); window.history.go(-1);</script>\n";
		exit;
	}
	if (count( $cid )) {
		mosArrayToInts( $cid );
