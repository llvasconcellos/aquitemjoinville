	...
	foreach($sections as $section) {
 	...
 	// get list of categories
  	if ( !$row->catid && !$row->sectionid ) {
 		$categories[] 		= mosHTML::makeOption( '-1', 'Escolher Categoria', 'id', 'name' );
...
function saveContent( $sectionid, $task ) {
	...
	switch ( $task ) {
		...
		case 'save':
		default:
			$msg = 'Item salvo com sucesso: '. $row->title;
...
function changeContent( $cid=null, $state=0, $option ) {
	...
	switch ( $state ) {
		...
		default:
			if ( $task == 'unarchive' ) {
				$msg = $total .' Item(s) Desarquivado(s) com sucesso';
			} else {
...
function toggleFrontPage( $cid, $section, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Selecione um artigo para alterar'); window.history.go(-1);</script>\n";
		exit;
	}
...
function removeContent( &$cid, $sectionid, $option ) {
	...
	$msg 	= $total ." Item(s) enviados para a lixeira";
...
function moveSectionSave( &$cid, $sectionid, $option ) {
	...
	if (!$newsect && !$newcat ) {
		mosRedirect( "index.php?option=com_content&sectionid=$sectionid&mosmsg=Ocorreu um erro" );
	}
