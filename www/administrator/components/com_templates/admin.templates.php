...
function saveTemplateSource( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operação falhou: Nenhum tema especificado.' );
	}
	if ( !$filecontent ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operação falhou: Conteúdo vazio.' );
	}
	...
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option , 'Operação falhou: '. $file .' não tem permissões de escrita.' );
	}
	...
	if ( $fp = fopen ($file, 'w' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operação falhou: Não foi possível abrir o arquivo para escrita.' );
	}
...
function saveTemplateCSS( $option, $client ) {
	...
	clearstatcache();
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operação falhou: O arquivo não pode ser salvo.' );
	}
