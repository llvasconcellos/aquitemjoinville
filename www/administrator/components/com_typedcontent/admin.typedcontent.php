...
function view( $option ) {
	...
	$ordering[] = mosHTML::makeOption( 'c.ordering ASC', 'Ordem crescente' );
	$ordering[] = mosHTML::makeOption( 'c.ordering DESC', 'Ordem decrescente' );
	$ordering[] = mosHTML::makeOption( 'c.id ASC', 'ID 0-9' );
	$ordering[] = mosHTML::makeOption( 'c.id DESC', 'ID 9-0' );
	$ordering[] = mosHTML::makeOption( 'c.title ASC', 'Título A-Z' );
	$ordering[] = mosHTML::makeOption( 'c.title DESC', 'Título Z-A' );
	$ordering[] = mosHTML::makeOption( 'c.created ASC', 'Data recente' );
	$ordering[] = mosHTML::makeOption( 'c.created DESC', 'Data antiga' );
	$ordering[] = mosHTML::makeOption( 'z.name ASC', 'Autor A-Z' );
	$ordering[] = mosHTML::makeOption( 'z.name DESC', 'Autor Z-A' );
	$ordering[] = mosHTML::makeOption( 'c.state ASC', 'Publicado recente' );
	$ordering[] = mosHTML::makeOption( 'c.state DESC', 'Publicado antigo' );
	$ordering[] = mosHTML::makeOption( 'c.access ASC', 'Mais Acessos' );
	$ordering[] = mosHTML::makeOption( 'c.access DESC', 'Menos Acessos' );
...
function edit( $uid, $option ) {
	...
	if ($uid) {
		...
		if ($row->isCheckedOut( $my->id )) {
			mosErrorAlert( "O módulo ".$row->title." está sendo editado atualmente por outro administrador" );
		}
...
function save( $option, $task ) {
	...
	if (trim( $row->publish_down ) == 'Nunca' || trim( $row->publish_down ) == '') {
	...
	switch ( $task ) {
		...
		case 'save':
			$msg = 'Artigo estático foi salvo com sucesso';
			mosRedirect( 'index2.php?option='. $option, $msg );
			break;

		case 'apply':
		default:
			$msg = 'As alterações no artigo estático foram salvas';
...
function trash( &$cid, $option ) {
	...
	$msg = $total ." Item(s) enviado para lixeira";
	mosRedirect( 'index2.php?option='. $option, $msg );
}
...
function changeState( $cid=null, $state=0, $option ) {
	...
	if ( $state == "1" ) {
		$msg = $total ." Item(s) publicado (s) com sucesso!";
	} else if ( $state == "0" ) {
		$msg = $total ." Item (s) despublicado (s) com sucesso!";
	}
...
function resethits( $option, $id ) {
	...
	$msg = 'Acessos reiniciados com sucesso';
	mosRedirect( 'index2.php?option='. $option .'&task=edit&hidemainmenu=1&id='. $row->id, $msg );
}
...
function menuLink( $option, $id ) {
	...
	$msg = $link .' (Link - Conteúdo estático) no menu: '. $menu .' criado com sucesso!';
