...
class JoomlapackArchiver extends JoomlapackObject
{
	...
	/**
	 * Initialises the archiver class. This method must be overriden in child
	 * classes and the parent's initialize() must be called at the END of the
	 * overriding method!
	 *
	...
	function _fwrite( $fp, $data )
	{
		...
		if($configuration->get("enableMySQLKeepalive"))
		{
			...
			if($endTime - $this->startTime > 0.5)
			{
				...
				JoomlapackTables::WriteVar('dummy', 1);
				//JoomlapackLogger::WriteLog(_JP_LOG_DEBUG, 'Keep-alive MySQL query on '.date('H:i:s'));			
			}
		}
	...
	*/
	 ...
	 * class to include each extracted file. Each call processes a SINGLE file from
	 * the installer. 
	 *
	 * @return array|bool False if an error occured, a return array otherwise
	 * @access public
	 */
