...
if (isset( $_POST['submit'] )) {
	...
	if($pass == NULL) {
		echo "<script>alert('Entre com sua senha'); document.location.href='index.php?mosmsg=Please enter a password'</script>\n";
		exit();
	}
	...
	if (@$my->id) {
		...
		if ( strcmp( $hash, $cryptpass ) || !$acl->acl_check( 'administration', 'login', 'users', $my->usertype ) ) {
			mosErrorAlert("Erro no nome de usuário, senha, ou nível de acesso.  Por favor, verifique os dados introduzidos e tente novamente", "document.location.href='index.php'");
		}
