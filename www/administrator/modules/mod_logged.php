...
foreach ( $rows as $row ) {
	if ( $acl->acl_check( 'administration', 'manage', 'users', $my->usertype, 'components', 'com_users' ) ) {
		...
		$name 	= '<a href="'. $link .'" title="Editar Usuários">'. $row->username .'</a>';
	} else {
		$name 	= $row->username;
	}
	?>
	<tr>
		...
		if ( $acl->acl_check( 'administration', 'manage', 'users', $my->usertype, 'components', 'com_users' ) ) {
			...
			<img src="images/publish_x.png" width="12" height="12" border="0" alt="Logout" Title="Forçar saída de usuário" />
			</a>
			</td>
			<?php
		}
