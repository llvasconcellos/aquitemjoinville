...
if (!defined( '_JOS_QUICKICON_MODULE' )) {
	...
	<div id="cpanel">
		...
		quickiconButton( $link, 'module.png', 'Adcionar novo conteúdo' );

		$link = 'index2.php?option=com_content&sectionid=0';
		quickiconButton( $link, 'addedit.png', 'Administrar Itens de Conteúdo' );

		$link = 'index2.php?option=com_typedcontent';
		quickiconButton( $link, 'addedit.png', 'Administrar Conteúdo Estático' );

		$link = 'index2.php?option=com_frontpage';
		quickiconButton( $link, 'frontpage.png', 'Administrar Página Principal' );

		$link = 'index2.php?option=com_sections&amp;scope=content';
		quickiconButton( $link, 'sections.png', 'Administrar Seção' );

		$link = 'index2.php?option=com_categories&amp;section=content';
		quickiconButton( $link, 'categories.png', 'Administrar Categoria' );

		$link = 'index2.php?option=com_media';
		quickiconButton( $link, 'mediamanager.png', 'Administrar Mídias' );

		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_trash';
			quickiconButton( $link, 'trash.png', 'Administrar Lixeira' );
		}

		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_menumanager';
			quickiconButton( $link, 'menu.png', 'Administrar Menus' );
		}

		if ( $my->gid > 24 ) {
			$link = 'index2.php?option=com_languages';
			quickiconButton( $link, 'langmanager.png', 'Administrar Idiomas' );
		}

		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_users';
			quickiconButton( $link, 'user.png', 'Administrar Usuários' );
		}

		if ( $my->gid > 24 ) {
			$link = 'index2.php?option=com_config&hidemainmenu=1';
			quickiconButton( $link, 'config.png', 'Configurações Globais' );
		}
