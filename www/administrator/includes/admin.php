...
function mosLoadCustomModule( &$module, &$params ) {
	...
	if ( $rssurl ) {
		if (!is_writable( $cachePath )) {
			echo '<tr>';
			echo '<td>Por favor, altere as permissões do diretório de cache para ser editável.</td>';
			echo '</tr>';
		} else {
...
function josSecurityCheck($width='95%') {
	...
	if ( RG_EMULATION != 0 ) {
		$wrongSettingsTexts[] = 'Configuração Joomla no arquivo globals.php - Emulação Joomla do Registo de Globais (RG_EMULATION) está `Ligado` em vez de `Desligado` <br /><span style="font-weight: normal; font-style: italic; color: #666;">Está `Ligado` por omissão mas apenas por questões de compatibilidade com algumas extensões.</span>';
	}
	...
	if ( count($wrongSettingsTexts) ) {
		...
		<div style="clear: both; margin: 3px; margin-top: 10px; padding: 5px 15px; display: block; float: left; border: 1px solid #cc0000; background: #ffffcc; text-align: left; width: <?php echo $width;?>;">
			<p style="color: #CC0000;">
				As seguintes Configurações de Segurança do Servidor PHP não estão otimizadas para <strong>SEGURANÇA</strong> e é recomendado que as altere:
			</p>
			...
			<p style="color: #666;">
				Por favor, consulte o <a href="http://www.joomla.org/security10" target="_blank" style="color: blue; text-decoration: underline">artigo oficial Joomla! sobre Segurança de Servidores</a> para mais informações.
			</p>
		</div>
		<?php
	}
