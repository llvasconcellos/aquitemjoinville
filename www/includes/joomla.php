...
class mosAbstractTasker {
	...
	function taskNotFound( $task ) {
		echo 'Tarefa ' . $task . ' não encontrada';
		return null;
	}
	/**
	...
	function methodNotFound( $name ) {
		echo 'Método ' . $name . ' não encontrado';
		return null;
	}
	/**
	...
	function initSessionAdmin($option, $task) {
		...
		if ($session_id == md5( $my->id . $my->username . $my->usertype . $logintime )) {
			...
			if (ini_get( 'session.auto_start' ) || !ini_get( 'session.use_cookies' )) {
				echo "<script>document.location.href='index.php?mosmsg=É necessário autenticar-se. Visite a Página Inicial. Se as configurações PHP session.auto_start estiver ligada ou a session.use_cookies estiver desligada, deverá corrigir estas situações para que seja possível a sua autenticação.'</script>\n";
			} else {
				echo "<script>document.location.href='index.php?mosmsg=É necessário autenticar-se. Visite a Página Inicial'</script>\n";
			}
			exit();
		} else {
			// session id does not correspond to required session format
			echo "<script>document.location.href='index.php?mosmsg=Sessão inválida'</script>\n";
			exit();
		}
...
class mosHTML {
  ...
  function writableCell( $folder, $relative=1, $text='', $visible=1 ) {
	$writeable 		= '<b><font color="green">Têm permissão para escrita</font></b>';
	$unwriteable 	= '<b><font color="red">Não têm permissão para escrita</font></b>';
...
class mosCategory extends mosDBTable {
	...
	*/
	...
	function check() {
		...
		if (trim( $this->title ) == '') {
			$this->_error = "A categoria deve possuir um título.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "A categoria deve ter um nome.";
			return false;
		}
		...
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Já existe uma categoria com esse nome. Indique um nome diferente e tente novamente.";
			return false;
		}
		return true;
	}
...
class mosSection extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "A seção deve possuir um título.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "A seção deve ter um nome.";
			return false;
		}
		...
		$xid = intval( $this->_db->loadResult() );
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Já existe uma seção com esse nome. Indique um nome diferente e tente novamente.";
			return false;
		}
		return true;
	}
...
class mosModule extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "O seu Módulo tem de possuir um título.";
			return false;
		}
...
class mosSession extends mosDBTable {
	...
	*/
	...
	function insert() {
		...
		if( !$ret ) {
			$this->_error = strtolower(get_class( $this ))."::erro ao salvar <br />" . $this->_db->stderr();
			return false;
		} else {
			return true;
		}
...
function mosWarning($warning, $title='Joomla! Warning') {
	...
	$tip 		.= '<img src="'. $mosConfig_live_site .'/includes/js/ThemeOffice/warning.png" border="0"  alt="Alerta"/></a>';
...
class mosAdminMenus {
	...
	*/
	function Target( &$row ) {
		$click[] = mosHTML::makeOption( '0', 'Mesma janela, com botões de navegação' );
		$click[] = mosHTML::makeOption( '1', 'Nova janela, com botões de navegação' );
		$click[] = mosHTML::makeOption( '2', 'Nova janela, sem botões de navegação' );
	...
	*/
	function Section( &$menu, $id, $all=0 ) {
		...
		$database->setQuery( $query );
		if ( $all ) {
			$rows[] = mosHTML::makeOption( 0, '- Todas as Seções -' );
	...
	*/
	function Images( $name, &$active, $javascript=NULL, $directory=NULL ) {
		...
		$images 	= array(  mosHTML::makeOption( '', '- Selecionar Imagem -' ) );
	...
	*/
	function UserSelect( $name, $active, $nouser=0, $javascript=NULL, $order='name', $reg=1 ) {
		...
		$database->setQuery( $query );
		if ( $nouser ) {
			$users[] = mosHTML::makeOption( '0', '- Sem Usuário -' );
...
class mosCommonHTML {
	...
	function ContentLegend( ) {
		...
		<tr align="center">
			<td>
			<img src="images/publish_y.png" width="12" height="12" border="0" alt="Pendente" />
			</td>
			<td>
			Publicado, mas está <u>Pendente</u> |
			</td>
			<td>
			<img src="images/publish_g.png" width="12" height="12" border="0" alt="Visível" />
			</td>
			<td>
			Publicado e é <u>Atual</u> |
			</td>
			<td>
			<img src="images/publish_r.png" width="12" height="12" border="0" alt="Terminado" />
			</td>
			<td>
			Publicado, mas <u>Expirou</u> |
			</td>
			<td>
			<img src="images/publish_x.png" width="12" height="12" border="0" alt="Terminado" />
			</td>
			<td>
			Não Publicado
			</td>
		...
		<tr>
			...
			Clique no ícone para alterar os estados.
			</td>
		</tr>
		</table>
		<?php
	...
	function menuLinksContent( &$menus ) {
		...
		foreach( $menus as $menu ) {
			...
			<tr>
				...
				<td>
				<strong>
				<a href="javascript:go2( 'go2menuitem', '<?php echo $menu->menutype; ?>', '<?php echo $menu->id; ?>' );" title="Ir para o item de menu">
				<?php echo $menu->name; ?>
				</a>
				</strong>
				</td>
			...
			<tr>
				...
				switch ( $menu->published ) {
					...
					case 1:
					default:
						echo '<font color="green">Publicado</font>';
						break;
				}
				?>
				</td>
	...
	function menuLinksSecCat( &$menus ) {
		...
		foreach( $menus as $menu ) {
			...
			<tr>
				...
				<td>
				<strong>
				<a href="javascript:go2( 'go2menuitem', '<?php echo $menu->menutype; ?>', '<?php echo $menu->id; ?>' );" title="Ir para item de menu">
				<?php echo $menu->name; ?>
				</a>
				</strong>
				</td>
	...
	*/
	...
	function PublishedProcessing( &$row, $i ) {
		...
		$alt 	= $row->published ? 'Publicado' : 'Não publicado';
...
class patHTML {
	...
	function yesNoRadio( &$tmpl, $template, $name, $value, $varname=null ) {
		$a = array(
			patHTML::makeOption( 0, 'Não' ),
			patHTML::makeOption( 1, 'Sim' )
		);
