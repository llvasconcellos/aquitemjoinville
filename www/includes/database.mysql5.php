...
class database {
	...
	*/
	function stderr( $showSQL = false ) {
		return "Falha de função da BD com o número de erro $this->_errorNum"
...
class mosDBTable {
	...
	*/
	function bind( $array, $ignore='' ) {
		if (!is_array( $array )) {
			$this->_error = strtolower(get_class( $this ))."::Falha de ligação.";
			return false;
		} else {
	...
	*/
	function store( $updateNulls=false ) {
		...
		if( !$ret ) {
			$this->_error = strtolower(get_class( $this ))."::falha ao arquivar <br />" . $this->_db->getErrorMsg();
			return false;
		} else {
			return true;
		}
...
//echo 'D: ' . $this->_db->getQuery();
	...
	*/
	function updateOrder( $where='' ) {
		...
		if (!array_key_exists( 'ordering', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ALERTA: ".strtolower(get_class( $this ))." não suporta ordenação.";
			return false;
		}
	...
	*/
	...
	function checkout( $user_id, $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ALERTA: ".strtolower(get_class( $this ))." não suporta checkouts.";
			return false;
		}
		$k = $this->_tbl_key;
		if ($oid !== null) {
	...
	function checkin( $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ALERTA: ".strtolower(get_class( $this ))." não suporta validação.";
			return false;
		}
	...
	*/
	...
	function publish( $cid=null, $publish=1, $user_id=0 ) {
		...
		if (count( $cid ) < 1) {
			$this->_error = "Nenhum item selecionado.";
			return false;
		}
