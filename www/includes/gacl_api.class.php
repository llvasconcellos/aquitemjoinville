...
class gacl_api extends gacl {
	...
	function return_page($url="") {
		...
		if (empty($url) AND !empty($_SERVER[HTTP_REFERER])) {
			$this->debug_text("return_page(): URL não definido, utilizando referência!");
	...
	function get_group_id($name = null, $group_type = 'ARO') {
		...
		if (empty($name) ) {
			$this->debug_text("get_group_id(): nome ($name) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if ($row_count > 1) {
			$this->debug_text("get_group_id(): $row_count linhas obtidas, mas apenas devia obter uma. Por favor, indique nomes únicos.");
			return false;
		}

		if ($row_count == 0) {
			$this->debug_text("get_group_id(): $row_count linhas obtidas");
			return false;
		}
	...
	function get_group_name($group_id = null, $group_type = 'ARO') {
		...
		if (!$group_id) {
			$this->debug_text("get_group_name(): group_id ($group_id) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if ($row_count > 1) {
			$this->debug_text("get_group_name(): $row_count linhas obtidas, mas apenas devia obter uma. Por favor, indique nomes únicos.");
			return false;
		}

		if ($row_count == 0) {
			$this->debug_text("get_group_name(): $row_count linhas obtidas");
			return false;
		}
	...
	function get_group_children($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		if (empty($group_id)) {
			$this->debug_text("get_group_children(): ID ($group_id) está vazio, mas é um dado obrigatório");
			return FALSE;
		}
	...
	function get_group_parents($group_id, $group_type = 'ARO', $recurse = 'NO_RECURSE') {
		...
		if (empty($group_id)) {
			$this->debug_text("get_group_parents(): ID ($group_id) está vazio, mas é um dado obrigatório");
			return FALSE;
		}
	...
	function add_group($name, $parent_id=0, $group_type='ARO') {
		...
		if (empty($name)) {
			$this->debug_text("add_group(): name ($name) OR parent id ($parent_id) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if ($parent_id == 0) {
			...
			if (count( $rs ) > 0) {
				$this->debug_text('add_group (): Já existe um grupo raiz .');
			...
			if (empty($parent_id)) {
				$this->debug_text("add_group (): id de dependência ($parent_id) está vazio, mas é um dado obrigatório");
				return FALSE;
			}
			...
			if (empty($rows)) {
				$this->debug_text('add_group (): ID de Dependência: '. $parent_id .' não encontrado.');
				// <mos> $this->db->RollBackTrans();
				return FALSE;
			}
			$row = $rows[0];
		...
		if (!$rs) {
			$this->debug_db('add_group: inserir registro');
			// <mos> $this->db->RollBackTrans();
			return FALSE;
		}
		...
		$this->debug_text('add_group (): Grupo adicionado como ID: '. $insert_id);
		return $insert_id;
	}
	...
	function get_group_objects($group_id, $group_type='ARO', $option='NO_RECURSE') {
		...
		if (empty($group_id)) {
			$this->debug_text("get_group_objects(): Group ID:  ($group_id) está vazio, mas é um dado obrigatório");
			return false;
		}
	...
	function add_group_object($group_id, $object_section_value, $object_value, $group_type='ARO') {
		...
		if (empty($group_id) OR empty($object_value) OR empty($object_section_value)) {
			$this->debug_text("add_group_object(): Group ID:  ($group_id) OR Value ($object_value) OR Section value ($object_section_value) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if (count( $rows ) != 1) {
			$this->debug_text("add_group_object (): Group ID ($group_id) OR Value ($object_value) OR Section value ($object_section_value) não é válido. Será que este objecto existe?");
			return FALSE;
		}
	...
	function del_group_object($group_id, $object_section_value, $object_value, $group_type='ARO') {
		...
		if (empty($group_id) OR empty($object_value) OR empty($object_section_value)) {
			$this->debug_text("del_group_object(): Group ID:  ($group_id) OR Section value: $object_section_value OR Value ($object_value) está vazio, mas é um dado obrigatório");
			return false;
		}

 		if (!$object_id = $this->get_object_id($object_section_value, $object_value, $group_type)) {
			$this->debug_text ("del_group_object (): Group ID ($group_id) OR Value ($object_value) OR Section value ($object_section_value) não é válido. Será que este objecto existe?");
			return FALSE;
		}
	...
	function del_group($group_id, $reparent_children=TRUE, $group_type='ARO') {
		...
		if (empty($group_id) ) {
			$this->debug_text("del_group(): ID de Grupo ($group_id) está vazio, mas é um dado obrigatório");
			return false;
		}
	...
	function get_object($section_value = null, $return_hidden=1, $object_type=NULL) {
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				$this->debug_text('get_object(): Tipo de objeto não válido: '. $object_type);
				return FALSE;
		}
...
//			SELECT		g.group_id,o.'. $group_type .'_id,(gm.group_id IS NOT NULL) AS member
	...
	function get_object_id($section_value, $value, $object_type=NULL) {
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				$this->debug_text('get_object_id(): Tipo de ojeto não válido: '. $object_type);
				return FALSE;
		}

		$this->debug_text("get_object_id(): Valor da Seção: $section_value Value: $value Object Type: $object_type");

		$section_value = trim($section_value);
		$value = trim($value);

		if (empty($section_value) AND empty($value) ) {
			$this->debug_text("get_object_id(): Valor da seção ($value) AND value ($value) está vazio, mas é um dado obrigatório");
			return false;
		}

		if (empty($object_type) ) {
			$this->debug_text("get_object_id(): Tipo de Objeto ($object_type) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if ($row_count > 1) {
			$this->debug_text("get_object_id(): $row_count linhas obtidas, mas apenas pode obter uma. Tal nunca deve acontecer, a base de dados não terá uma chave única.");
			return false;
		}

		if ($row_count == 0) {
			$this->debug_text("get_object_id(): $row_count linhas obtidas");
			return false;
		}
	...
	function add_object($section_value, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {
		...
		switch(strtolower(trim($object_type))) {
			...
			case 'axo':
				...
				break;
			default:
				$this->debug_text('add_object(): Tipo de Objeto Inválido: '. $object_type);
				return FALSE;
		}

		$this->debug_text("add_object(): Valor da Seção: $section_value Valor: $value Ordem: $order Nome: $name Tipo de Objeto: $object_type");
		...
		if (empty($name) OR empty($section_value) ) {
			$this->debug_text("add_object(): nome ($name) OR valor de seção ($section_value) está vazio, mas é um dado obrigatório");
			return false;
		}

		if (strlen($name) >= 255 OR strlen($value) >= 230 ) {
			$this->debug_text("add_object(): nome ($name) OR valor ($value) é demasiado longo.");
			return false;
		}

		if (empty($object_type) ) {
			$this->debug_text("add_object(): Tipo de Objeto ($object_type) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if (count( $rows ) != 1) {
			// Section is invalid
			$this->debug_text("add_object(): Valor de Seção: $section_value Object Type ($object_type) não existe, mas é um dado obrigatório");
			return false;
		}
		...
		$this->debug_text("add_object(): Adicionado um objeto como ID: $insert_id");
		return $insert_id;
	}
	/*======================================================================*\
	...
	function edit_object($object_id, $section_value, $name, $value=0, $order=0, $hidden=0, $object_type=NULL) {
		...
		$this->debug_text("edit_object(): ID: $object_id, Valor da Seção: $section_value, Valor: $value, Ordem: $order, Nome: $name, Object Tipo: $object_type");
		...
		if (empty($object_id) OR empty($section_value) ) {
			$this->debug_text("edit_object(): ID Objeto ($object_id) OR Valor de Seção ($section_value) está vazio, mas é um dado obrigatório");
			return false;
		}

		if (empty($name) ) {
			$this->debug_text("edit_object(): nome ($name) está vazio, mas é um dado obrigatório");
			return false;
		}

		if (empty($object_type) ) {
			$this->debug_text("edit_object(): Tipo de Objeto ($object_type) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		$this->debug_text('edit_object(): Modificado '. strtoupper($object_type) .' ID: '. $object_id);

		if ($old[0] != $value OR $old[1] != $section_value) {
			$this->debug_text("edit_object(): Valor OR Valor de Seção Alterados, atualize as restantes tabelas.");
		...
		if (!$this->db->getErrorNum()) {
			...
			$this->debug_text ('edit_object(): Mapa de Valores modificado : '. $value .' Section Value: '. $section_value);
		}
	...
	function del_object($object_id, $object_type=NULL, $erase=FALSE) {
		...
		$this->debug_text("del_object(): ID: $object_id Object Type: $object_type, Apague todos os objetos referenciados: $erase");

		if (empty($object_id) ) {
			$this->debug_text("del_object(): Object ID ($object_id) está vazio, mas é um dado obrigatório");
			return false;
		}

		if (empty($object_type) ) {
			$this->debug_text("del_object(): Object Type ($object_type) está vazio, mas é um dado obrigatório");
			return false;
		}
		...
		if (empty($object)) {
			$this->debug_text('del_object(): O objeto especificado ('. strtoupper($object_type) .' ID: '. $object_id .') não foi encontrado.<br />SQL = '.$this->db->stderr());
			return FALSE;
		}
		...
		if ( ( isset($acl_ids) AND $acl_ids !== FALSE ) OR ( isset($groups_ids) AND $groups_ids !== FALSE) ) {
			...
			$this->debug_text("del_object(): Não é possível eliminar o objeto já que está a ser referenciado por GROUPs (".@implode($group_ids).") or ACLs (".@implode($acl_ids,",").")");
