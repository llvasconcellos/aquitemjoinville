...
*/
	...
	*/
	function _form_mos_section( $name, $value, &$node, $control_name ) {
		...
		array_unshift( $options, mosHTML::makeOption( '0', '- Escolher Seção -', 'id', 'title' ) );
	...
	*/
	function _form_mos_category( $name, $value, &$node, $control_name ) {
		...
		$database->setQuery( $query );
		$options = $database->loadObjectList();
		array_unshift( $options, mosHTML::makeOption( '0', '- Escolher Categoria -', 'id', 'title' ) );
	...
	*/
	function _form_mos_menu( $name, $value, &$node, $control_name ) {
		...
		array_unshift( $options, mosHTML::makeOption( '', '- Escolher Menu -' ) );
	...
	*/
	function _form_filelist( $name, $value, &$node, $control_name ) {
		...
		if ( !$node->getAttribute( 'hide_none' ) ) {
			array_unshift( $options, mosHTML::makeOption( '-1', '- '. 'Não Utilizar' .' -' ) );
		}
		if ( !$node->getAttribute( 'hide_default' ) ) {
			array_unshift( $options, mosHTML::makeOption( '', '- '. 'Utilizar Padrão' .' -' ) );
		}
