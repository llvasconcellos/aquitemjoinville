...
function openFont($font){
  ...
  if (file_exists($dir.'php_'.$name.'.afm')){
	$this->addMessage('openFont: Arquivo php já existe '.$dir.'php_'.$name.'.afm');
	...
	if (!isset($this->fonts[$font]['_version_']) || $this->fonts[$font]['_version_']<1){
	  ...
	  $this->addMessage('openFont: Limpar, possibilitar uma nova versão.');
	  unset($this->fonts[$font]);
	}
  }
  if (!isset($this->fonts[$font]) && file_exists($dir.$name.'.afm')){
	// then rebuild the php_<font>.afm file from the <font>.afm file
	$this->addMessage('openFont: Construir arquivo php a partir de '.$dir.$name.'.afm');
	$data = array();
	...
	$fp = @fopen($dir.'php_'.$name.'.afm','w') or die ("Por favor, certifique-se de que o diretório \"media\" está como editável (CHMOD 777).");
	fwrite($fp,serialize($data));
	fclose($fp);
	mosChmod($dir.'php_'.$name.'.afm');
  } else if (!isset($this->fonts[$font])){
	$this->addMessage('Abrir Fonte: Não foram encontrados arquivos');
...
//	  $ttffile = substr($fontName,0,strlen($fontName)-4).'.ttf';
	  $this->addMessage('selectFont: a verificar - '.$fbfile);
	  if (substr($fontName,-4)=='.afm' && strlen($fbtype) ){
		$adobeFontName = $this->fonts[$fontName]['FontName'];
//		$fontObj = $this->numObj;
		$this->addMessage('selectFont: Adicionar arquivo de fonte - '.$fbfile.' - '.$adobeFontName);
		...
		$this->addMessage('adicionar informação extra a fonte.('.$fontObj.')');
		...
		$this->addMessage('Selecionar Fonte: arquivo pfb ou ttf não encontrados. Carregue OK caso seja uma das 14 fontes standard');
	  }
...
function addPngFromFile($file,$x,$y,$w=0,$h=0){
  ...
  if ($fp){
	...
	$error = 1;
	$errormsg = 'problemas ao abrir o arquivo: '.$file;
  }
  set_magic_quotes_runtime($tmp);
  ...
  if (!$error){
	...
	if (substr($data,0,8)!=$header){
	  $error=1;
	  $errormsg = 'este arquivo não possui um cabeçalho válido';
	}
  }
...
//print_r($info);
		  ...
		  if ($info['compressionMethod']!=0){
			$error=1;
			$errormsg = 'método de compressão não suportado';
		  }
		  if ($info['filterMethod']!=0){
			$error=1;
			$errormsg = 'método de filtro não suportado';
		  }
		  break;
		case 'PLTE':
...
//			$transparency['grayscale']=$this->PRVT_getBytes($data,$p+8,2); // g = grayscale
	...
	if(!$haveHeader){
	  $error = 1;
	  $errormsg = 'ausência do cabeçalho de informação';
	}
	if (isset($info['interlaceMethod']) && $info['interlaceMethod']){
	  $error = 1;
	  $errormsg = 'Não existe suporte para imagens interlaçadas na criação de pdf.';
	}
  }

  if (!$error && $info['bitDepth'] > 8){
	$error = 1;
	$errormsg = 'apenas é suportado 8 bits de profundidade ou inferior';
  }

  if (!$error){
	if ($info['colorType']!=2 && $info['colorType']!=0 && $info['colorType']!=3){
	  $error = 1;
	  $errormsg = 'Não existe suporte para transparência em canais alpha. A transparência apenas é suportada em imagens de palete até 256 cores.';
	} else {
  ...
  if ($error){
	$this->addMessage('Erro de PNG - ('.$file.') '.$errormsg);
	return;
  }
  if ($w==0){
...
function addImage(&$img,$x,$y,$w=0,$h=0,$quality=75){
  ...
  if ($fp){
	...
	$error = 1;
	$errormsg = 'problemas ao abrir o arquivo';
  }
