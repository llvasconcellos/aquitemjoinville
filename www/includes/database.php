...
class database {
	...
	*/
	function stderr( $showSQL = false ) {
		return "Falha de função da Base de dados com o número de erro $this->_errorNum"
...
class mosDBTable {
	...
	*/
	function bind( $array, $ignore='' ) {
		if (!is_array( $array )) {
			$this->_error = strtolower(get_class( $this ))."::falha de arquivo.";
			return false;
		} else {
	...
	*/
	function store( $updateNulls=false ) {
		...
		if (!$ret) {
			$this->_error = strtolower(get_class($this))."::falha ao salvar <br />" . $this->_db->getErrorMsg();
			return false;
		} else {
			return true;
		}
